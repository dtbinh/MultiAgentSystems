...
to take-action
  if(length history > memory-size)[
    set history butlast history
    ]
  set history fput ([pcolor] of neighbors) history 
  let past-colors reduce [sentence ?1 ?2] history 
  
  ;stochastic chioce based on past history
...
; set pcolor one-of modes past-colors 
 ...
 ;The Least color
 ;let color-frequency map[count-matches ? past-colors] colors 
 ;let min-color-count min color-frequency 
 ;set pcolor item (position min-color-count color-frequency) colors ;Index
 
 
 
   
  ;;fictious, take a stochastic action based on history 
